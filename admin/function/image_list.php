<?php
	...
	if(is_file($filePath)) echo 'yessssss  '.$filePath.'               ';
	...
	/* 	$images = $dataDom -> getElementsByTagName('image')->item(0);
		...
		foreach($items as $item){
			...
			$filePath = '../data/upload/'.$fileCat.'/'.$fileId.'.'.$fileExt;
			$listeimg .= '<li class="jcarousel-item-'.$i.'"><img src="' . $filePath . '" width="96px" height="96px" alt="'.$item -> textContent.'" title="'.$fileTitle.'" /></li>';
			$i++;
		}
		$listeimg .= '</ul>';
		echo $listeimg;
