...
class ModuleAdminRubrique extends ModuleAdmin{
	...
	public function setData(){
		...
		if(isset($this -> _urlQuery['id']) && !empty($this -> _urlQuery['id'])){
			$idRub = $this -> _urlQuery['id'];	// = lg,chapter,nom-du-chapitre
			$idArray = explode(',',$idRub);
			/* 
			// PAGE object
			...
			/* if(self::$_page -> error){
				...
				$this -> _menu -> deleteNodeId($idRub);	// supprime du menu
				Xml_Page::removeId($idRub);				// supprime du flux, tags etc.
				FrontController::redirect('');
				//echo '<br/>pbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb';
			} */
			self::$arrayToParse['FORM_ID'] =	$idRub;
	...
	public function editRub($idRub){
		...
		self::$arrayToParse['CURRENT_CRUMB'] = 'Rubrique : '.self::$arrayToParse['FORM_TITRE'];
	...
	//  EDITING CHAPTER
	//-------------------------
	public function deleteChap($idRub){	
		self::$arrayToParse['FORM_ID'] =	$idRub;
			
		$infos = $this -> _menu -> getMenuItem($idRub);
		if(!empty($infos)){		// si le chapitre est trouvé dans le menu
			$this -> parseArray($infos,'form_'); 
			
			self::$arrayToParse['CURRENT_CRUMB'] = 'Rubrique : '.self::$arrayToParse['FORM_NOM'];
			
			if(isset($_POST) and !empty($_POST)){
				if(isset($_POST['ok']) && $_POST['ok'] == 'ok'){	
					// SUPPRESSION DU CHAPITRE DU MENU
					/* $pageObject -> delete(); 			// delete file */
					$this -> _menu 	-> deleteNodeId($idRub, true);	// delete menu entry
				...
				$this->_templateNameModule = 'page';
				$this -> formTarget($this->urlAddQuery(array('id'=>$idRub, 'action' => 'delete'),true), 'FICHIER_CIBLE');
			}
		}else{	// redirection vers la page d'accueil
			FrontController::redirect('');
		}
...
/* 	
	...
	public function createContent(){		
		...
		if($idFileNew != false){
			...
			$idRub = $lg.','.$this->_module.','.$idFileNew;
			...
			$this -> _menu -> addMenuItem($lg, $array, $idRub);
			// redirection vers la page d'édition
			FrontController::redirect('fr,page,editer.html?id='.$idRub);
		}else{
			echo 'Pas créer!!!';
		}
	} */
