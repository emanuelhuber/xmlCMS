...
class Lister{
	...
	public function addEntry($id, $title, $link, $summary, $right, $publication){//, $langue, $module){
		$this -> deleteEntry($id);
			
			// htmlentities($oldData['description'], ENT_QUOTES, "UTF-8")
	...
	// Nouvelle entrée
		...
		$doc = $this -> _dom -> getElementsByTagName('feed') -> item(0);
			...
			$nodeSummary = $this -> _dom -> createElement("right");
			$entry -> appendChild($nodeSummary);
			//$nodeSummary -> setAttribute('type','html');
			$textSummary = $this -> _dom -> createTextNode($right); 
			$nodeSummary -> appendChild($textSummary); 

			$nodeSummary = $this -> _dom -> createElement("publication");
			$entry -> appendChild($nodeSummary);
			//$nodeSummary -> setAttribute('type','html');
			$textSummary = $this -> _dom -> createTextNode($publication); 
			$nodeSummary -> appendChild($textSummary);
			
		/* 	$nodeSummary = $this -> _dom -> createElement("langue");
			$entry -> appendChild($nodeSummary);
			//$nodeSummary -> setAttribute('type','html');
			$textSummary = $this -> _dom -> createTextNode($langue); 
			$nodeSummary -> appendChild($textSummary); 
			
			$nodeSummary = $this -> _dom -> createElement("module");
			$entry -> appendChild($nodeSummary);
			//$nodeSummary -> setAttribute('type','html');
			$textSummary = $this -> _dom -> createTextNode($module); 
			$nodeSummary -> appendChild($textSummary);  */
