...
class ModuleLogin extends ModuleSquelette{
	...
	public function __construct($url, $query, $urlArray, $conf){
		...
		self::$myPage = new Xml_User($urlArray, $root="root");
	}
	...
	public function setData(){
		...
		/* $this -> echo_r( self::$myPage -> usersList());
		 */
		if($this -> _urlQuery['action'] == 'out'){
			// indiquer que l'utilisateur s'est bien déconnecté et quand!
			self::$myPage -> logOutUser($_SESSION['login_visiteur']['username']);
			echo "yes";
			...
			// on pourrait aussi vérifier le http_referer (mais pas forcément fiable)
				...
				if((time() - $_SESSION['token_time']) <60*5){	// si le temps pour remplir le formulaire n'excède pas 5 min
					/* echo $_POST['username'];
					echo '<br/>';
					$user = self::$myPage -> getUser($_POST['username']);
					echo $user; */
					...
					/* 	echo '<br/> user find!<br/>';
						...
						if($user['username'] == $_POST['username'] && $user['password'] == $_POST['password']){	
							...
							if(self::$myPage -> logInUser($_POST['username'])){
								...
								$_SESSION['login_visiteur']=array();
								//$_SESSION['login_visiteur']['password']=$userPassword;
								$_SESSION['login_visiteur']['username']=$user['username'];
								$_SESSION['login_visiteur']['droits']=$user['droits'];
								...
								//$_SESSION['login_visiteur']['IP']= $_SERVER['REMOTE_ADDR'];
								$_SESSION['login_visiteur']['nav']= $_SERVER['HTTP_USER_AGENT'];
								$_SESSION['login_visiteur']['connection']= true;
								$_SESSION['login_visiteur']['initiated']= true;
								
								// SI $_SESSION['login_visiteur']['logout']==0
								// ATTENTION L'UTILISATEUR NE S'EST PAS DECONNECTE
								// LORS DE SA SESSION PRECEDANTE !
								$_SESSION['login_visiteur']['logout'] = $user['logout'];
								
								$_SESSION['login_visiteur']['group'] = $user['group'];
								$_SESSION['login_visiteur']['groupname'] = $user['groupname'];
								$_SESSION['login_visiteur']['date'] = $user['date'];
								$_SESSION['login_visiteur']['timestamp'] = $user['timestamp'];
								//$_SESSION['login_visiteur']['nom']=$donnees['nom'];
				...
				if(isset($formulaire_valide) && $formulaire_valide==false){
					//self::$arrayToParse['MESSAGE'] .=  '<br/>wait!!!<br/>';
					sleep(2);
				}
			}else{
				$this-> setRandomToken();
